...
const out=path.join(root,'docs/qa/garden-scenic-2026-09-13');
...
await page.goto('http://127.0.0.1:8784/docs/qa/garden-scenic-2026-09-13/');
...
await check('six views render; house stays in exterior review frames',async()=>{
 const frames={};for(const view of ['arrival','overview','house','veranda','shore','mobile']){await page.locator('[data-view='+view+']').click();await page.waitForTimeout(100);frames[view]=await page.evaluate(()=>window.gardenScenic);assert.equal(frames[view].view,view);if(view!=='veranda'){const b=frames[view].houseBounds;assert(b[0]>=0&&b[1]>=0&&b[2]<=1&&b[3]<=1,view+JSON.stringify(b));}}
 await fs.writeFile(path.join(out,'verified-frames.json'),JSON.stringify(frames,null,2));return Object.keys(frames);
});
...
await check('unavailable WebGL exposes fallback without unhandled exception',async()=>{
 const p=await browser.newPage();await p.addInitScript(()=>{const original=HTMLCanvasElement.prototype.getContext;HTMLCanvasElement.prototype.getContext=function(type,...args){return /webgl/.test(type)?null:original.call(this,type,...args);};});await p.goto('http://127.0.0.1:8784/docs/qa/garden-scenic-2026-09-13/');await p.waitForFunction(()=>window.gardenScenicError);assert(await p.locator('#fallback').isVisible());await p.screenshot({path:path.join(out,'no-webgl.png')});await p.close();
});
