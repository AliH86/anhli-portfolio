...
const out=process.env.GARDEN_TEST_OUT||path.join(root,'docs/qa/garden-stable-2026-09-14/regression');
...
await check('MAP available on all states; dialog focus stays inside and returns',async()=>{
  ...
  await page.keyboard.press('m');await page.locator('#dgMap .dg-map-destination[data-dg-route="music"]').click();assert.equal(await page.locator('#dgApp').getAttribute('data-route'),'music');
