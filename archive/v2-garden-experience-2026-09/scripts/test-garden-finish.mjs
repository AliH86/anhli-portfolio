...
const out=path.join(root,'docs/qa/garden-finish-2026-09-14');
...
await page.goto('http://127.0.0.1:8784/docs/qa/garden-finish-2026-09-14/');
...
await check('exported geometry has water 0.65m below finished veranda',async()=>{
 const result=await page.evaluate(async()=>{const T=await import('/js/vendor/three/three.module.js');const {assets,layout}=window.scenicInspect();let water;assets['landscape-finish'].traverse(o=>{if(o.isMesh&&/water/i.test(o.material?.name))water=o;});const box=new T.Box3().setFromObject(water,true);return {waterMin:box.min.y,waterMax:box.max.y,porch:layout.porchFinishedLevel};});
...
await check('unavailable WebGL exposes fallback without unhandled exception',async()=>{
 const p=await browser.newPage();await p.addInitScript(()=>{const original=HTMLCanvasElement.prototype.getContext;HTMLCanvasElement.prototype.getContext=function(type,...args){return /webgl/.test(type)?null:original.call(this,type,...args);};});await p.goto('http://127.0.0.1:8784/docs/qa/garden-finish-2026-09-14/');await p.waitForFunction(()=>window.gardenScenicError);assert(await p.locator('#fallback').isVisible());await p.screenshot({path:path.join(out,'no-webgl.png')});await p.close();
});
